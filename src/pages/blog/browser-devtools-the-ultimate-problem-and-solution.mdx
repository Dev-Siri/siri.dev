...
```ts
async function new(formData: FormData) {
  "use server";
...
Huh, it could be a file? Interesting. Maybe if I just you know, just assert the type to something else, it will fix it:

```ts
email as string;
```

There we go. Let me add another field which is a picture

```html
<input type="file" name="logo" />
```

Oof, why is TypeScript inferring it as FormDataEntryValue again? Let me fix that:

```ts
logo as File;
```

Aight enough. So what you observed is an average JS developer trying TS. Here is the thing, the Native API definitions give you a hint on why you are wrong, this is important, **do not ignore this `FormDataEntryValue` returned from `form.get`**.
...
Around 2 years ago, a YouTube user found that the YouTube channel creation didn't do server-side validation. Its input minlength in the HTML (Client) was set to 50 characters. He went into the devtools and removed the minlength. And guess what??? He successfully got a username that is over 50 chars.

And they actually didn't add any validation until weeks later but till then, it was too late and [JSAL](https://www.youtube.com/watch?v=iqMrGb0MaKY) (btw sub to him so he can get more playbuttons) already made a channel, and he even tried to get a playbutton.

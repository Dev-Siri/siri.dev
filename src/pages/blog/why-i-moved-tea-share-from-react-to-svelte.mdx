...
app without a massive client-side JavaScript bundle and get some sleep because my mind doesn't have to think of the big client component I left in the app.
...
errors in it. In React, I remember before server components (RSCs don't hydrate so no problem using them), there were a lot of problems with hydration
when showing a formatted time. If there was a post published 10s ago, on the server it would generate 10s Ago, but by the time the client receives the
generated HTML, obviously the time would be now around 11-12s ago, but since the server generated HTML still has 10s, it throws a hydration error because
we need both server & client data to be idempotent to hydrate the page correctly. But in Svelte, it doesn't matter. Since there is no component tree, the
hydration process in SvelteKit is only attaching event-listeners to stuff, & there was no need for extra overhead of a VDOM.

## Things I hate about Svelte

Svelte's isn't all great tbh. Its very nice to work with but still a little rough around the edges.

### The templating language

The templating language of Svelte is kind of annoying tbh. Its definitely concise, readable & gets the job done, but I really hate it. Its the #1 thing I hate about Svelte. For example, lets take React && JSX vs Svelte.
